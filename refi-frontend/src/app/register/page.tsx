...
const Register: NextPage = () => {
  ...
  const handleFileChange = (e: ChangeEvent<HTMLInputElement>): void => {
    ...
    if (files && files[0]) {
      ...
      const allowedTypes = {
        verification_certificate: ['.pdf', '.png', '.jpg', '.jpeg'],
        before_images: ['image/*'],
        after_images: ['image/*'],
        nft_image: ['image/*']
      };
      ...
      const isValidType = allowedTypes[name as keyof typeof allowedTypes]?.some(type => {
        if (type === 'image/*') {
          return fileType.startsWith('image/');
        }
        return type === `.${fileType.split('/')[1]}`;
      });

      if (!isValidType) {
        alert(`Invalid file type for ${name}. Please upload a valid file.`);
        return;
      }
        ...
        return (
          <div className="space-y-8">
            ...
            <div className="bg-gray-800/50 p-6 rounded-lg border border-gray-700">
              <h3 className="text-xl font-semibold text-white mb-4">NFT Summary</h3>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <p className="text-gray-400">Total Carbon Offset</p>
                  <p className="text-2xl font-bold text-white">{formData.total_co2_offset || '0'} tons</p>
                </div>
                <div>
                  <p className="text-gray-400">Impact Percentage</p>
                  <p className="text-2xl font-bold text-white">{calculateImpactPercentage()}%</p>
                </div>
                <div>
                  <p className="text-gray-400">Carbon Offset Value</p>
                  <p className="text-2xl font-bold text-white">${calculateCarbonOffsetValue()}</p>
                </div>
                <div>
                  <p className="text-gray-400">Suggested NFT Price Range</p>
                  <p className="text-2xl font-bold text-white">
                    ${Number(calculateCarbonOffsetValue()) * 1.5} - ${Number(calculateCarbonOffsetValue()) * 2}
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-6">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-300 mb-2">
                  NFT Price (SOL) <span className="text-red-500">*</span>
                </label>
                <input
                  type="number"
                  name="nft_price"
                  ...
                  onChange={handleChange}
                  min={Number(calculateCarbonOffsetValue()) * 1.5}
                  step="0.1"
                  className="w-full px-4 py-2 bg-gray-800 border border-gray-700 rounded-lg focus:outline-none text-white"
                ...
                <p className="mt-1 text-sm text-gray-400">
                  Minimum price: {Number(calculateCarbonOffsetValue()) * 1.5} SOL
                </p>
              </div>
                    ...
                    {formData.nft_image_url ? (
                      ...
                      <>
                        ...
                        <div className="flex text-sm text-gray-400">
                          <label
                            htmlFor="nft_image"
                            className="relative cursor-pointer bg-gray-800 rounded-md font-medium text-blue-500 hover:text-blue-400 focus:outline-none"
                          >
                  ...
                  <div className="mb-8">
                    <div className="flex justify-between mb-2">
                      <div className="flex items-center">
                        ...
                        <span className="ml-2 text-sm text-gray-300">Organization</span>
                      </div>
                      <div className="flex items-center">
